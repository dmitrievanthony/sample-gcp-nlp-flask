...
def historical_links():
    result = list(map(lambda row: {'link': row['link'], 'date': row['date'], 'title': row['title'], 'sentiment_score': json.loads(row['sentiment_analysis_result'])['documentSentiment']['score']}, historical_data))
